    ...
    <>
      ...
      <main className="mx-auto max-w-5xl px-4 sm:px-6 py-10">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h1 className="text-[clamp(28px,4.4vw,38px)] font-extrabold tracking-[-0.035em]">Tikuvchi paneli</h1>
          ...
          {FILTERS.map((f) => (
            <button
              ...
              className={`rounded-full px-4 py-2 text-sm font-medium transition ring-1 ${
                filter === f.value
                  ? "bg-grad text-accent-fg ring-transparent"
...
function StatCard({ label, value }: { label: string; value: number }) {
  return (
    <div className="card">
      ...
      <div className="mt-2 text-[clamp(28px,4.4vw,38px)] font-extrabold tracking-[-0.035em]">{value}</div>
    </div>
  );
}
